...
function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 45; i++) {
      particles.push({
        x: Math.random() * 1000, y: Math.random() * 800, vx: 0, vy: -0.2,
        size: 12, alpha: 0, char: KANJI[i % KANJI.length], type: "kanji",
    ...
    <div className="min-h-screen flex">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm font-light text-stone-500 tracking-wide">To learn is to live.</p>
          <div className="mt-8 h-px w-12 bg-stone-700" />
        </div>

        <div className="relative z-10 grid grid-cols-3 gap-4">
          {[{ value: "10k+", label: "Words" },{ value: "4", label: "Topics" },{ value: "AI", label: "Powered" }].map(({ value, label }) => (
            <div key={label}>
              <p className="text-lg font-extralight text-white/70 tracking-tight">{value}</p>
      ...
      <div className="flex-1 relative flex items-center justify-center px-6 py-12 overflow-hidden">
        ...
        >
          ...
          <motion.div {...fadeUp(0.12)} className="space-y-3 mb-4">
            <StoneInput value={email} onChange={setEmail} placeholder="Email address" icon={Mail} />
