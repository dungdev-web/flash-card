          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="absolute bottom-3 left-3 right-10">
                <p className="text-white text-base font-extralight tracking-wide drop-shadow">
                  {word}
                </p>
                {meaning && (
                  <p className="text-white/65 text-xs font-light mt-0.5">
                    {meaning}
                  </p>
                )}
              </div>
              ...
              {imgData?.author && (
                <a
                  href={imgData.authorUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="absolute bottom-2 right-2.5 text-[9px] text-white/35 tracking-wide hover:text-white/60 transition-colors"
                >
                  {imgData.author} · Unsplash
                </a>
              )}
            </motion.div>
          </AnimatePresence>
...
function WordStatsPanel({
  ...
  if (!word)
    return (
      <div
        className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-4 flex flex-col items-center justify-center gap-2"
        style={{ minHeight: 100 }}
      >
        <BookOpen
          className="w-5 h-5 text-stone-300 dark:text-stone-600"
  ...
  return (
    <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-3.5">
      <div className="flex items-baseline gap-2 flex-wrap mb-2 pb-2 border-b border-stone-100 dark:border-stone-800">
        <span className="text-lg font-extralight text-stone-800 dark:text-stone-100 tracking-tight">
          {word}
        </span>
        {transcription && (
      ...
      {meaning && (
        <p className="text-sm text-stone-500 dark:text-stone-400 font-light mb-2.5">
          {meaning}
        </p>
      )}
      <div className="grid grid-cols-3 gap-2 mb-2.5">
        {[
          ...
          <div
            key={label}
            className="text-center py-1.5 rounded-lg bg-stone-50 dark:bg-stone-800"
          >
            <p className="text-sm font-light text-stone-700 dark:text-stone-200">
              {value}
            </p>
            <p className="text-[9px] uppercase tracking-[1px] text-stone-300 dark:text-stone-600 mt-0.5">
              {label}
            </p>
          </div>
        ))}
  ...
  return (
    <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-3.5 flex-1">
      ...
      {word ? (
        ...
        >
          <div className="flex flex-wrap gap-1">
            {word.split("").map((ch, i) => (
              <span
                key={i}
                className="inline-flex items-center justify-center w-6 h-6 rounded-md bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-xs font-light text-stone-700 dark:text-stone-300"
              >
                {ch === " " ? "·" : ch}
              </span>
            ))}
          ...
          <div className="space-y-1.5 pt-1">
            {[
              {
                label: "Associate",
                text: meaning ? `Link "${word}" → "${meaning}" visually` : "—",
              },
              { label: "Repeat", text: "Say 3× aloud with eyes closed" },
              {
                label: "Use it",
                text: example
                  ? "Write 1 personal sentence today"
                  : "Generate an example first",
              },
            ].map(({ label, text }) => (
              <div key={label} className="flex items-start gap-2">
                <div className="w-1 h-1 rounded-full bg-stone-300 dark:bg-stone-600 mt-1.5 flex-shrink-0" />
                <p className="text-[11px] text-stone-500 dark:text-stone-400 leading-relaxed">
                  <span className="text-[10px] uppercase tracking-[1.5px] text-stone-400 dark:text-stone-500">
                    {label} ·{" "}
                  </span>
                  {text}
                </p>
        ...
        <p className="text-[11px] text-stone-300 dark:text-stone-600 italic">
          Enter a word to see tips.
        </p>
      )}
    </div>
  );
...
export default function AddWordPage() {
  ...
  useEffect(() => {
    setWordType(english.trim().includes(" ") ? "phrase" : "word");
  }, [english]);
  ...
  const simpleTranscription = (w: string) =>
    w
      ...
      .replace(/e$/g, "")
      .split("")
      .map((c) => ({ a: "æ", e: "ɛ", i: "ɪ", o: "ɒ", u: "ʌ", y: "aɪ" })[c] ?? c)
      .join("");
    ...
    fetch(`/api/transcription?word=${encodeURIComponent(english)}`)
      ...
      .then((d) => {
        ...
        if (d.meanings?.length) {
          setAvailableParts(d.meanings);
          setSelectedPart(0);
        } else setAvailableParts([]);
      })
      .catch(() => {
  ...
  const currentPartOfSpeech =
    wordType === "phrase"
      ? "phrase"
      : availableParts[selectedPart]?.partOfSpeech || "";
  ...
  const handleSubmit = async () => {
    // Giới hạn free user
    if (isFree && wordCount !== null && wordCount >= WORD_LIMIT) {
      router.push("/upgrade");
      return;
    }
    if (!english || !meaning) {
      ...
      <div className="h-[calc(100vh-57px)] flex flex-col px-6 py-5 max-w-6xl mx-auto overflow-hidden">
        ...
        >
          <div>
            ...
            <h1 className="text-2xl font-extralight tracking-tight text-stone-800 dark:text-stone-100">
              New Entry
            </h1>
          </div>
          <div className="flex items-center gap-1.5">
            {["Word", "Context", "Topic"].map((s, i) => {
              const done =
                i === 0 ? !!english && !!meaning : i === 1 ? !!example : true;
              return (
                <div key={s} className="flex items-center gap-1.5">
                  <div
                    className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] tracking-wide transition-all duration-300 ${done ? "bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900" : "border border-stone-200 dark:border-stone-700 text-stone-400 dark:text-stone-500"}`}
                  >
                    {done && (
                  ...
                  {i < 2 && (
                    <div className="w-4 h-px bg-stone-200 dark:bg-stone-700" />
                  )}
                </div>
              );
            })}
          </div>
          {isFree && wordCount !== null && (
            <div
              className={`text-[10px] px-2.5 py-1 rounded-full border transition-colors
    ${
      wordCount >= WORD_LIMIT
        ? "border-red-200 dark:border-red-800 text-red-400"
        : wordCount >= 40
          ? "border-amber-200 dark:border-amber-800 text-amber-500"
        ...
        <div className="grid grid-cols-3 gap-3 flex-1 min-h-0">
          ...
          >
            <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-3.5">
              <Label
                icon={wordType === "phrase" ? MessageSquare : Globe}
                aside={
                  english ? (
                    <span className="text-[10px] px-2 py-0.5 rounded-full border border-stone-200 dark:border-stone-700 text-stone-400 dark:text-stone-500 uppercase tracking-wide">
                      {wordType}
                    </span>
                  ) : null
                }
            ...
            <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-3.5">
              ...
              <StoneInput
                value={meaning}
                onChange={setMeaning}
                placeholder="Meaning appears automatically..."
              />
            </div>
            ...
            <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-3.5 flex flex-col flex-1">
              ...
              <div className="flex-1">
                <StoneTextarea
                  value={example}
                  onChange={setExample}
                  placeholder="Click 'AI Generate' or write your own..."
                />
              </div>
              {example && english && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="mt-2 pt-2 border-t border-stone-100 dark:border-stone-800"
                >
                  <p className="text-[10px] uppercase tracking-[1.5px] text-stone-300 dark:text-stone-600 mb-1">
                    Preview
                  </p>
                  <p className="text-xs text-stone-500 dark:text-stone-400 leading-relaxed">
                    {example
                      .split(new RegExp(`(${english})`, "gi"))
                      .map((part, i) =>
                        part.toLowerCase() === english.toLowerCase() ? (
                          <mark
                            key={i}
                            className="bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 rounded px-0.5 not-italic"
                          >
                            {part}
                          </mark>
                        ) : (
            ...
            <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-xl p-3.5">
              ...
              <div className="grid grid-cols-6 gap-1">
                {TOPICS.map((t) => (
                  ...
                  >
                    <p
                      className={`text-[10px] font-light leading-none mb-0.5 ${topic === t.value ? "text-white dark:text-stone-900" : "text-stone-700 dark:text-stone-300"}`}
                    >
                      {t.value}
                    </p>
                    <p
                      className={`text-[9px] ${topic === t.value ? "text-stone-400 dark:text-stone-600" : "text-stone-300 dark:text-stone-600"}`}
                    >
                      {t.labelJa}
                    </p>
                  </button>
              ...
              <AnimatePresence mode="wait">
                {success ? (
                  ...
                  >
                    ...
                    {wordType === "phrase" ? "Phrase" : "Word"} Added
                  </motion.span>
                ) : saving ? (
                  <motion.span
                    key="saving"
