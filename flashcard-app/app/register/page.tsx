...
function StoneInput({
  ...
  return (
    <div className="relative">
      <Icon
        className="absolute text-black left-3 top-1/2 -translate-y-1/2 w-4 h-4 "
        strokeWidth={1.5}
      />
      <input
        type={type}
    ...
    >
      <div className="flex gap-1 mb-1">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className={`flex-1 h-[2px] rounded-full transition-all duration-300 ${i < score ? colors[score] : "bg-stone-200 dark:bg-stone-700"}`}
          />
        ))}
      </div>
      <p className="text-[10px] uppercase tracking-[1.5px] text-stone-400 dark:text-stone-500">
        {labels[score]}
      </p>
    </motion.div>
  );
  ...
  return (
    <div className="min-h-screen flex">
      ...
      >
        ...
        <div className="relative z-10">
          <p className="text-[10px] uppercase tracking-[3px] text-stone-700">
            桜 · Sakura AI
          </p>
        </div>
      </motion.div>
