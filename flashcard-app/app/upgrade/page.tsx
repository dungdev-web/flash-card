...
function SealMark({ char, color }: { char: string; color: string }) {
  ...
  return (
    <div
      className={`w-12 h-12 rounded-full border-2 flex items-center justify-center text-lg font-light ${colors[color]}`}
...
export default function UpgradePage() {
  ...
  const { isPro, isMaster, isAdmin } = useRole();
  return (
    <AuthGuard>
      <div className="min-h-[calc(100vh-57px)] bg-white dark:bg-stone-950 px-6 py-12">
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div {...fadeUp(0)} className="text-center mb-16">
            ...
            <h1
              className="text-5xl font-extralight tracking-tight text-stone-800 dark:text-stone-100 mb-3"
              style={{
          ...
          <div className="grid grid-cols-3 gap-5 items-stretch">
            {plans.map((plan, idx) => {
              // 2. Tính toán logic Disabled
              // - Nếu là Master/Admin: khóa tất cả (vì đã ở đỉnh cao nhất)
              // - Nếu là Pro: khóa Free và Pro, chỉ mở Master
              // - Nếu là Free: khóa Free, mở Pro và Master
              ...
              if (isMaster || isAdmin) {
                if (plan.id === "master") isCurrentPlan = true;
                isDisabled = true;
              } else if (isPro) {
                if (plan.id === "pro") isCurrentPlan = true;
                if (plan.id === "free" || plan.id === "pro") isDisabled = true;
              } else {
                // User thường (Free)
                if (plan.id === "free") {
                  isCurrentPlan = true;
              ...
              return (
                ...
                >
                  ...
                  } ${isCurrentPlan ? "ring-2 ring-stone-400 dark:ring-stone-500 ring-offset-4 dark:ring-offset-stone-950" : ""}`}
                  >
                    <div className="p-6 flex flex-col flex-1">
                      {/* ... SealMark và badge giữ nguyên ... */}
                      {/* Top: seal + badge */}
                      <div className="flex items-start justify-between mb-5">
                        <SealMark char={plan.tier[0]} color={plan.color} />
                        {plan.featured && (
                      ...
                      <div className="my-4">
                        <div className="flex items-baseline gap-1">
                          {plan.price !== "0" && (
                            <span className="text-xs text-stone-400 dark:text-stone-500 mb-1">
                              $
                            </span>
                          )}
                          <span
                      ...
                      <div className="mt-auto">
                        {/* Nếu là gói hiện tại hoặc bị khóa, có thể bọc Link hoặc dùng div để tránh điều hướng */}
                        {isDisabled ? (
                          <button
                            disabled
                            className="w-full h-11 rounded-xl text-sm font-light tracking-wide border border-stone-200 dark:border-stone-700 text-stone-400 dark:text-stone-500 bg-stone-50/50 dark:bg-stone-800/30 cursor-default"
                          >
                            {ctaText}
                          </button>
                        ) : (
          ...
          <motion.div {...fadeUp(0.45)} className="mt-14 text-center space-y-3">
            <div className="flex items-center justify-center gap-3">
              ...
              <p className="text-[10px] uppercase tracking-[4px] text-stone-300 dark:text-stone-600">
                保証
              </p>
              <div className="h-px w-24 bg-stone-100 dark:bg-stone-800" />
            </div>
