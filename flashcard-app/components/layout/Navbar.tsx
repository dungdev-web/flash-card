...
const navItems = [
  { href: "/dashboard",            label: "Dashboard",  labelJa: "概要",      icon: LayoutDashboard, num: "01" },
  { href: "/dashboard/flashcards", label: "Flashcards", labelJa: "単語カード", icon: BookOpen,        num: "02" },
  { href: "/dashboard/add-word",   label: "Add Word",   labelJa: "追加",      icon: Plus,            num: "03" },
  { href: "/dashboard/topics",     label: "Topics",     labelJa: "トピック",  icon: BookOpen,        num: "04" },
];
    ...
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[999]">
          ...
          >
            <motion.div className="mb-14 text-center pointer-events-none"
              initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: CONTENT_DELAY + 0.07 }}>
              <p className="text-[10px] uppercase tracking-[4px] text-stone-600 mb-1">学習アプリ</p>
              <p className="text-lg font-extralight tracking-[3px] text-white/80">FlashCard</p>
            </motion.div>
              ...
              {navItems.map((item, idx) => {
                ...
                return (
                  <motion.div key={item.href}
                    ...
                    transition={{ delay: CONTENT_DELAY + 0.1 + idx * 0.07, duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>
                    <button onClick={() => handleNav(item.href)} className="w-full text-left">
                      <div className={`relative flex items-center gap-4 px-5 py-4 rounded-xl transition-all duration-200
                        ${isActive
                        ...
                        <span className="text-[10px] text-stone-700 tracking-widest">{item.labelJa}</span>
                        <span className="text-[10px] text-stone-700 tabular-nums ml-2">{item.num}</span>
                      </div>
                    </button>
                  </motion.div>
                );
            ...
            <motion.div className="mt-14 flex flex-col items-center gap-4 pointer-events-auto"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <motion.div className="absolute top-0 left-0 bottom-0 w-1/2 z-20"
            initial={{ x: "-100%" }}
            animate={{ x: ["-100%", "0%", "-100%"] }}
            exit={{ x: 0 }}
            transition={{ duration: 1.6, times: [0, 0.62, 1], ease: [0.4, 0, 0.2, 1] }}>
            <ShojiPanel side="left" />
          </motion.div>

          {/* Right shoji door: slides in from right → closes → opens back right */}
          <motion.div className="absolute top-0 right-0 bottom-0 w-1/2 z-20"
            initial={{ x: "100%" }}
            animate={{ x: ["100%", "0%", "100%"] }}
            exit={{ x: 0 }}
            transition={{ duration: 1.6, times: [0, 0.62, 1], ease: [0.4, 0, 0.2, 1] }}>
          ...
          <motion.div className="absolute top-0 bottom-0 left-1/2 w-[2px]  z-20"
            initial={{ opacity: 0 }}
  ...
  return (
    <>
      <motion.nav
        ...
        className="relative border-b border-stone-200 dark:border-stone-800 bg-white/80 dark:bg-stone-950/80 backdrop-blur-xl"
      >
        <motion.div
        ...
        <div className="flex items-center justify-between px-5 py-3.5 max-w-5xl mx-auto">
          ...
          <div className="text-center">
            <p className="text-[9px] uppercase tracking-[3px] text-stone-400 dark:text-stone-500 leading-none mb-0.5">学習</p>
            <p className="text-sm font-light tracking-[2px] text-stone-700 dark:text-stone-300">FlashCard</p>
          </div>
